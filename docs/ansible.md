...
- `ansible/playbooks/reconcile-infra-nodes.yml`
  - consumes `ansible/inventory/generated/infra_nodes.yml`
  - bootstraps Docker on infra manager/worker nodes
  - joins nodes to Swarm with the declared role
  - applies required Swarm labels such as `kind`
